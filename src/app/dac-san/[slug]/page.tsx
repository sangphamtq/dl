...
export default async function SpecialtyPublicPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  ...
  return (
    <div className="flex flex-1 flex-col">
      <SiteHeader />
      <ListingViewTracker type="specialty" id={specialty.id} />

      <main className="flex-1">
        <section className="relative h-[340px] w-full sm:h-[420px]">
          <Image
            src={heroUrl}
            alt={specialty.images[0]?.alt ?? specialty.name}
            fill
            priority
            sizes="100vw"
          ...
          <div className="absolute inset-0">
            <div className="mx-auto flex h-full max-w-5xl flex-col justify-end px-4 pb-10 text-white sm:px-6 sm:pb-12">
              <nav className="flex flex-wrap items-center gap-1 text-sm text-white/80">
                ...
                <ChevronRight className="size-3.5" aria-hidden />
                <Link
                  href={`/diem-den/${specialty.place.slug}`}
        ...
        <RelatedPosts type="specialty" id={specialty.id} />
      </main>

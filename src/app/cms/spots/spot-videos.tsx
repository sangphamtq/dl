...
export function SpotVideosManager({
  spotId,
  videos,
}: {
  spotId: string;
  ...
  function onAdd() {
    ...
    startTransition(async () => {
      const res = await addVideo(spotId, input, caption);
      if (!res.ok) {
        setError(res.error);
        return;
      }
    ...
    <li className="flex items-center gap-3 rounded-xl border p-2">
        ...
        <Input
          ...
          onBlur={() => {
            if ((video.caption ?? "") !== caption.trim())
              run(() => updateVideoCaption(video.id, spotId, caption));
          }}
      ...
      <div className="flex flex-col">
        <Button
          ...
          onClick={() => run(() => moveVideo(video.id, spotId, -1))}
          aria-label="Lên"
        >
      ...
      <AlertDialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Gỡ video khỏi danh sách của địa điểm. Không xóa video gốc trên TikTok.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={(e) => {
                e.preventDefault();
                run(() => deleteVideo(video.id, spotId).then(() => setConfirmOpen(false)));
              }}
              disabled={pending}

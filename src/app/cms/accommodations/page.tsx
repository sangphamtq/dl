...
import { AccommodationRowActions } from "./row-actions";
...
export default async function AccommodationsPage({
  ...
  const category =
    sp.category && ACCOMMODATION_CATEGORIES.some((c) => c.value === sp.category)
      ? sp.category
      : "all";
  ...
  return (
    <div className="p-6 sm:p-8">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">Lưu trú</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            Khách sạn, homestay, resort… nên ở.
          </p>
        </div>
        <Link href="/cms/accommodations/new" className={cn(buttonVariants())}>
      ...
      <div className="mt-6 flex flex-col gap-3">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <form className="relative" action="/cms/accommodations">
            {status !== "all" && (
        ...
        {hasFilters && (
          <div>
            <ClearFilters href="/cms/accommodations" show={hasFilters} />
          </div>
        )}
      </div>
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="truncate text-xs text-muted-foreground">
                    {a.place.name}
                    {a.category &&
                      ` · ${labelOf(ACCOMMODATION_CATEGORIES, a.category)}`}
                  </p>
                </div>
                <Badge
          ...
          <div className="px-4 py-16 text-center">
            <p className="text-sm text-muted-foreground">
              {q || status !== "all" || category !== "all" || place !== "all"
                ? "Không tìm thấy cơ sở nào khớp bộ lọc."

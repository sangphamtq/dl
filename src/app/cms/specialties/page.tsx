...
export default async function SpecialtiesPage({
  ...
  const filters: Filters = { status, place, q, page };
  const hasFilters = status !== "all" || place !== "all" || q !== "";
...
async function SpecialtyList({ filters }: { filters: Filters }) {
  const { status, place, q, page } = filters;
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="truncate text-xs text-muted-foreground">
                    {s.place.name} · {s._count.eateries} quán
                  </p>
                </div>
                <Badge
